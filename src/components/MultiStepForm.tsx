...
export function MultiStepForm({ formSource = "hero" }: MultiStepFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.nombre || !formData.telefono || !formData.acceptedPolicies) return;
    ...
    const payload = {
      ...
      especialidad: "No especificada (Valoración inicial)",
  ...
  const canProceed = formData.nombre.trim().length > 0 && formData.telefono.trim().length > 0 && formData.acceptedPolicies;
